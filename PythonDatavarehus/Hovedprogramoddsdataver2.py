...
from Maxoddsdata import Maxoddsdata
import os
maxdato = []
...
if os.path.isfile("D:\workplace_python\PythonDatavarehus\DNTodds.txt"):
    print("D:\workplace_python\PythonDatavarehus\DNTodds.txt","blir slettet")
    os.remove("D:\workplace_python\PythonDatavarehus\DNTodds.txt")    
...
maxdato = Maxoddsdata(maxdato)
testdato = [maxdato]
#maxdato  = ['Fredag, 1. jan 2021']
print(maxdato,"Max dato har verdien hoved program")
SQL = '''\
SELECT Dato
  FROM [DWtrav].[dbo].[Resultat-datoer]
  Where Datoinn > ?
  and Bane like ' Jarlsberg-travbane'
'''     
cursor = con.cursor()
cursor.execute(SQL,(testdato))
